...
import { Planet, ZodiacSign, AspectType, AdditionalPointType } from '@/domain/chart/enums'
...
describe('CaelusChartMapper', () => {
  it('maps a complete chart to NatalChart with all components', () => {
    const chart = CaelusChartMapper.map(mockChart as any, 'test-birth-123')
  ...
  it('maps Sun position correctly', () => {
    const chart = CaelusChartMapper.map(mockChart as any, 'test')
  ...
  it('maps North Node and South Node as additional points', () => {
    const chart = CaelusChartMapper.map(mockChart as any, 'test')
    const northNode = chart.additionalPoints.find(ap => ap.point === AdditionalPointType.NORTH_NODE)
    const southNode = chart.additionalPoints.find(ap => ap.point === AdditionalPointType.SOUTH_NODE)!
  ...
  it('should map Lilith from extra bodies', () => {
    ...
    const result = CaelusChartMapper.map(chart as any, 'test')
  ...
  it('should map Part of Fortune from Hermetic lots', () => {
    const chart = { ...mockChart }
    chart.lots = [{ lot: 'fortune', lon: 45, sign: 'taurus', signDeg: 15, house: 2 }]
    const result = CaelusChartMapper.map(chart as any, 'test')
  ...
  it('handles empty bodies gracefully', () => {
    ...
    const chart = CaelusChartMapper.map(empty as any, 'test')
  ...
  it('handles missing nodes', () => {
    ...
    const chart = CaelusChartMapper.map(noNodes as any, 'test')
